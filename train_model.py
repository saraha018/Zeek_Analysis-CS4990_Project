...
def prepare_data(df):
    ...
    exclude_cols = ['label', 'pcap_name', 'sample_id']
...
def train_model(X_train, y_train, X_test, y_test, feature_names):
    ...
    Returns:
        Trained model, scaler, metrics dictionary
    """
    # Scale features
...
def main():
    parser = argparse.ArgumentParser(
        description="Train ML model for network traffic classification"
    )
    parser.add_argument(
        '--features',
        type=str,
    ...
    # Split data - handle case with too few samples for stratified split
    unique_classes = np.unique(y)
    min_class_count = min([(y == cls).sum() for cls in unique_classes])
    
    if len(X) < 4 or min_class_count < 2:
        # Not enough samples for train/test split
        print(f"\n⚠️  WARNING: Only {len(X)} samples ({min_class_count} per class minimum)")
        print("   Cannot perform train/test split. Using all data for training.")
        print("   Model performance metrics will be limited.")
        print("   Consider adding more samples for better results.")
        
        X_train, X_test = X, X
        y_train, y_test = y, y
        
        print(f"\nUsing all {len(X)} samples for training (no test set)")
    else:
